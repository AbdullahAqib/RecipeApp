...
@Composable
fun RecipeScreen (modifier: Modifier = Modifier, categoriesState: MainViewModel.RecipeState, navigateToCategoryDetail:(Category)->Unit) {
    ...
    Box (modifier = Modifier.fillMaxSize()) {
        when{
            ...
            categoriesState.error != null ->
                ...
                CategoryScreen(categories = categoriesState.list, navigateToCategoryDetail)
        }
    }
}
...
fun CategoryScreen(categories : List<Category>, navigateToCategoryDetail:(Category)->Unit) {
    LazyVerticalGrid(columns = GridCells.Fixed(2), modifier = Modifier.fillMaxSize()) {
        items(categories) {
            category ->
            CategoryItem(category, navigateToCategoryDetail)
        }
    }
}
...
fun CategoryItem(category: Category, navigateToCategoryDetail:(Category)->Unit) {
    Column (modifier = Modifier
        .fillMaxSize()
        .padding(8.dp)
        .clickable { navigateToCategoryDetail(category) },
